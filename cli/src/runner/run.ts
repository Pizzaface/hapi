...
const DEFAULT_CODEX_HOME = () => process.env.CODEX_HOME ?? join(os.homedir(), '.codex');

const formatCodexConfigOverrides = (overrides: string[]): string => {
  const lines = overrides
    .map((entry) => entry.trim())
    .filter((entry) => entry.length > 0);
  if (lines.length === 0) {
    return '# Generated by hapi\n';
  }
  return `# Generated by hapi\n${lines.join('\n')}\n`;
};
...
const copyCodexAuthIfPresent = async (sourceDir: string, destDir: string): Promise<void> => {
  try {
...
export async function startRunner(): Promise<void> {
  ...
  try {
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      const worktreeBranch = options.worktreeBranch;
      const codexConfigOverrides = Array.isArray(options.codexConfigOverrides)
      ...
      try {
        ...
        let resolvedCodexHomeDir: string | undefined;
        let createdCodexHomeDir = false;

        if (agent === 'codex') {
          resolvedCodexHomeDir = options.codexHomeDir
            ?? (options.resumeSessionId ? codexHomeBySessionId.get(options.resumeSessionId) : undefined);

          const needsCodexHome = Boolean(resolvedCodexHomeDir)
            || Boolean(options.token)
            || Boolean(codexConfigOverrides && codexConfigOverrides.length > 0);

          if (!resolvedCodexHomeDir && needsCodexHome) {
            resolvedCodexHomeDir = await fs.mkdtemp(join(os.tmpdir(), 'hapi-codex-'));
            createdCodexHomeDir = true;
          }

          if (resolvedCodexHomeDir) {
            extraEnv = {
              ...extraEnv,
              CODEX_HOME: resolvedCodexHomeDir
            };
          }
        }
        ...
        if (agent === 'codex' && resolvedCodexHomeDir) {
          if (!options.token && createdCodexHomeDir) {
            await copyCodexAuthIfPresent(DEFAULT_CODEX_HOME(), resolvedCodexHomeDir);
          }
          if (codexConfigOverrides) {
            await writeCodexConfigFile(resolvedCodexHomeDir, codexConfigOverrides);
          }
        }
